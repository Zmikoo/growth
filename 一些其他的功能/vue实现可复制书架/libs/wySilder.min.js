(function (b) {
    ...
    a.prototype._init = function (c, d) {
        ...
        b(this).each(function (g, j) {
            var k = b(this), i = k.find(".js-silder-ctrl"), l = k.find(".silder-box");
            k.addClass("js-silder");
            if (l.length == 0) {
                var h = b('<div class="silder-box"></div></div>');
                l = h.children(".silder-box");
                k.append(l);
                j.silderMain = l
            } else {
                j.silderMain = l;
            ...
            j.imgsLen = j.silderMain.children(".silder-page").length;
            var f = k.find(".silder-page");
            f.each(function (m, n) {
        ...
        var covers=document.getElementsByClassName("bk-cover0");
        for(var i=0;i<covers.length;i++){
            var urlinfo="books/covers/"+BookInfos[0][i].book_cover;
            covers[i].style="background-image:"+"url(" + urlinfo + ");"
        }
    };
    a.prototype._calcSilderMainWH = function (g, e, c) {
        if (g && g.length && g.length > 0) {
            // var f = g[0], h = f.silderMain, d = c || h.children(".silder-page").eq(0).find("img").height();
            var f = g[0], h = f.silderMain, d = c || h.children(".silder-page").eq(0).find("img").height()|| h.children(".silder-page").eq(0).find("div").height();
            console.log(h.children(".silder-page").eq(0).find("img").height()), h.height(d)
        } else {
            b(this).each(function (i, k) {
                // var l = k.silderMain, j = c || l.children(".silder-page").eq(0).find("img").height();
                var l = k.silderMain, j = c || l.children(".silder-page").eq(0).find("img").height()|| l.children(".silder-page").eq(0).find("div").height();
                l.height(j)
            })
        }
    };
    a.prototype._createSilderImg = function () {
        ...
        b(c).each(function (f, g) {
            ...
            b.each(d, function (l, j) {
                var k = j.alt ? j.alt : "";
                silderMainImg = b('<div class="silder-page"></div>'), eleA = null, img = b('<img src="' + j.img + '" alt="' + k + '">');
                if (l == 0) {
    ...
    a.prototype._createControllBtns = function () {
        ...
        b(c).each(function (h, e) {
            var k = b(this), f = this.silderMain, l = this.silderCtrl, m = f.children(".silder-page").length, g = 0,
                d = "";
    ...
    a.prototype._ctrlBtnBindEvents = function () {
        ...
        b(c).each(function (d, e) {
            this.silderCtrl.children().on("click", function () {
                var f = b(e).width(), h = e.silderMain.children(".silder-page");
                if (b(this).hasClass("silder-ctrl-next")) {
                    ...
                    if (b(this).hasClass("silder-ctrl-prev")) {
                        ...
                        var covers=document.getElementsByClassName("bk-cover");
                        for(var i=0;i<covers.length;i++){
                            covers[i].style="";
                        }
                        var covers=document.getElementsByClassName("bk-cover"+e.index);
                        for(var i=0;i<covers.length;i++){
                            var urlinfo="books/covers/"+BookInfos[e.index][i].book_cover;
                            covers[i].style="background-image:"+"url(" + urlinfo + ");"
                        }
                    } else {
                        ...
                        var covers=document.getElementsByClassName("bk-cover"+g);
                        for(var i=0;i<covers.length;i++){
                            var urlinfo="books/covers/"+BookInfos[g][i].book_cover;
                            covers[i].style="background-image:"+"url(" + urlinfo + ");"
                        }
                    }
    ...
    a.prototype._play = function (c) {
        var f = this, d = c[0], g = d.imgsLen, e = c.width(), h = d.silderMain.children(".silder-page");
        ...
        //加载当前页面数据
        // alert($(".bk-cover"+g))
        var covers=document.getElementsByClassName("bk-cover"+d.index);
        for(var i=0;i<covers.length;i++){
            var urlinfo="books/covers/"+BookInfos[d.index][i].book_cover;
            covers[i].style="background-image:"+"url(" + urlinfo + ");"
        }
